...
function ServersPageContent() {
  ...
  useEffect(() => { if (!actionParam) { suppressActionRef.current = false; return; } if (suppressActionRef.current) return; if (actionParam === "create" && !modalOpen) openCreate(false); if ((actionParam === "edit" || actionParam === "delete") && !servers.length) return; if (actionParam === "edit" && serverParam && editing?._id !== serverParam) { const server = servers.find((item) => item._id === serverParam); if (server) openEdit(server, false); } if (actionParam === "delete" && serverParam && deleting?._id !== serverParam) { const server = servers.find((item) => item._id === serverParam); if (server) setDeleting(server); } }, [actionParam, deleting?._id, editing?._id, modalOpen, serverParam, servers]); // eslint-disable-line react-hooks/exhaustive-deps, react-hooks/set-state-in-effect
