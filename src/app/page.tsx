...
function currentProject(project?: string | null) {
  return (
    project ??
    ...
    async (options?: { silent?: boolean }) => {
      ...
      try {
        ...
        const res = await fetch(
          `/api/servers${projectId ? `?projectId=${projectId}` : ""}`,
          { cache: "no-store" },
        );
  ...
  async function checkNow(id: string) {
    ...
    try {
      ...
      const serverRes = await fetch(
        `/api/servers/${id}${projectId ? `?projectId=${projectId}` : ""}`,
        { cache: "no-store" },
      );
      if (!serverRes.ok)
        throw new Error(
              ...
              <div className="space-y-3 p-3">
                <div>
                  <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2">
                    <Link
                      href={projectHref(`/servers/${server._id}`)}
