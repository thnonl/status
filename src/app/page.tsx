...
function DashboardPageContent() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const projectId = currentProject(searchParams.get("project"));
  ...
  useEffect(() => {
    const serverId = searchParams.get("history");
    if (!serverId || historyServer?._id === serverId) return;
    const server = servers.find((item) => item._id === serverId);
    if (server) void openHistory(server, false);
  }, [historyServer?._id, searchParams, servers]); // eslint-disable-line react-hooks/exhaustive-deps

  function closeOverlays() { setLightbox(null); setHistoryServer(null); setQuery({ screenshot: null, history: null }); }
  ...
  async function openHistory(server: ServerDto, updateUrl = true) {
    if (updateUrl) setQuery({ history: server._id });
