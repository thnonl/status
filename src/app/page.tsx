...
import { Activity, GripVertical, Clock, Globe2, Info, Pencil, Plus, Server } from "lucide-react";
...
function DashboardPageContent() {
  ...
  async function clearHistory() {
    if (!historyServer || !window.confirm(`Clear all history for ${historyServer.name}?`)) return;
    setClearingHistory(true);
    setHistoryError("");
    try {
      const res = await fetch(`/api/servers/${historyServer._id}/history`, { method: "DELETE" });
      if (!res.ok) throw new Error((await res.json()).error ?? "Clear history failed");
      setHistoryItems([]);
      setHistoryServer((server) => server ? { ...server, latestCheck: null, uptime24h: null, uptime10d: null } : server);
      setServers((items) => items.map((server) => server._id === historyServer._id ? { ...server, latestCheck: null, uptime24h: null, uptime10d: null } : server));
  ...
  return <main className="space-y-3">
    ...
    <section className="grid grid-cols-[repeat(auto-fill,minmax(260px,1fr))] gap-3">{filteredServers.map((server) => {
      ...
      return <article key={server._id} draggable onDragStart={(event) => { setDraggingId(server._id); event.dataTransfer.effectAllowed = "move"; }} onDragOver={(event) => { event.preventDefault(); event.dataTransfer.dropEffect = "move"; }} onDrop={() => dropServer(index)} onDragEnd={() => setDraggingId("")} className={`overflow-hidden rounded-xl border bg-white/[0.04] shadow-xl shadow-black/20 transition ${draggingId === server._id ? "scale-[0.98] border-cyan-400/60 opacity-60" : "border-white/10"}`}>
        ...
        <p className="text-xs text-slate-500">Last checked: {fmt(server.latestCheck?.checkedAt)}</p><div className="grid grid-cols-2 gap-2"><button onClick={() => checkNow(server._id)} disabled={checkingIds.includes(server._id)} className="rounded-xl border border-white/10 px-3 py-2 text-sm text-slate-200 hover:bg-white/10 disabled:cursor-wait disabled:opacity-60">{checkingIds.includes(server._id) ? "Checking..." : "Check now"}</button><Link href={projectHref(`/servers/${server._id}`)} className="inline-flex items-center justify-center gap-2 rounded-xl border border-white/10 px-3 py-2 text-sm text-slate-200 hover:bg-white/10"><Info size={15}/> Details</Link></div></div>
      </article>})}</section>

  {editing ? (<Modal title="Edit server" onClose={() => setEditing(null)}><form onSubmit={saveEdit} className="space-y-3"><input required placeholder="Name" value={editForm.name} onChange={(e) => setEditForm({ ...editForm, name: e.target.value })} className="w-full rounded-xl border border-white/10 bg-black/30 px-3 py-2 text-white"/><input required placeholder="https://example.com" value={editForm.url} onChange={(e) => setEditForm({ ...editForm, url: e.target.value })} className="w-full rounded-xl border border-white/10 bg-black/30 px-3 py-2 text-white"/><input placeholder="Health route (optional, default /health, fallback URL)" value={editForm.healthRoute} onChange={(e) => setEditForm({ ...editForm, healthRoute: e.target.value })} className="w-full rounded-xl border border-white/10 bg-black/30 px-3 py-2 text-white"/><input placeholder="Screenshot route (default server URL)" value={editForm.screenshotRoute} onChange={(e) => setEditForm({ ...editForm, screenshotRoute: e.target.value })} className="w-full rounded-xl border border-white/10 bg-black/30 px-3 py-2 text-white"/><textarea placeholder="Description" value={editForm.description} onChange={(e) => setEditForm({ ...editForm, description: e.target.value })} className="w-full rounded-xl border border-white/10 bg-black/30 px-3 py-2 text-white"/><input placeholder="tags" value={editForm.tags} onChange={(e) => setEditForm({ ...editForm, tags: e.target.value })} className="w-full rounded-xl border border-white/10 bg-black/30 px-3 py-2 text-white"/>{editError && <p className="text-rose-300">{editError}</p>}<div className="flex justify-end gap-3"><button type="button" onClick={() => setEditing(null)} className="rounded-xl border border-white/10 px-3 py-2 text-slate-200">Cancel</button><button className="rounded-xl bg-cyan-400 px-3 py-2 font-semibold text-slate-950">Save</button></div></form></Modal>) : null}
