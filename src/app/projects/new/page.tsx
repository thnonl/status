...
export default function NewProjectPage() {
  ...
  if (done) return <main className="space-y-3"><p className="text-lg text-emerald-300">Project “{done.name}” created.</p><Link href={`/?project=${done._id}`} className="text-cyan-300">Go to Dashboard</Link></main>;
  return <main className="mx-auto max-w-xl space-y-3"><h1 className="text-2xl font-semibold text-white">New project</h1><form onSubmit={submit} className="space-y-3 rounded-xl border border-white/10 bg-white/[0.04] p-3"><input required placeholder="Project name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="w-full rounded-xl border border-white/10 bg-black/30 px-3 py-2 text-white"/><input placeholder="slug (auto)" value={form.slug} onChange={(e) => setForm({ ...form, slug: e.target.value })} className="w-full rounded-xl border border-white/10 bg-black/30 px-3 py-2 font-mono text-white"/><textarea placeholder="Description" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} className="w-full rounded-xl border border-white/10 bg-black/30 px-3 py-2 text-white"/>{error && <p className="text-rose-300">{error}</p>}<div className="flex justify-end gap-3"><Link href="/projects" className="rounded-xl border border-white/10 px-3 py-2 text-slate-200">Cancel</Link><button className="rounded-xl bg-cyan-400 px-3 py-2 font-semibold text-slate-950">Create</button></div></form></main>;
}
